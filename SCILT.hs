module SCILT (SCILabFml(..), LTNode(..), ltIsProvable, startNode) where
...
instance LTSystem LTNode where
    ltSuccessors = SCILT.ltSuccessors
    ltIsAxiomatic = SCILT.ltIsAxiomatic
    ltIsProven = SCILT.ltIsAxiomatic
